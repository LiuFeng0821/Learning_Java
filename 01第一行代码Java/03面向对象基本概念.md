...
在Java中this可以干三件事情：
1. **调用本类属性**
2. **调用本类方法**：使用**this调用构造方法**时要放在**构造方法的首行**。
3. **表示当前对象**

## 3.11 引用传递
* 引用传递的核心意义：**同一块堆内存空间可以被不同的栈内存所指向，不同栈内存可以对同一块堆内存进行内容的修改。**
...
## 3.14 static关键字
* **static属性**：类中所有对象都共用这个属性，另外在没有实例化对象产生的情况下，依然可以使用static属性，可以通过**类名称直接调用**。
* **static定义方法**

## 3.15 代码块
* **普通代码块**：如果一个**代码块写在方法里**，就称它为普通代码块。
* **构造块**：如果将一个**代码块写在一个类里**，这个代码块就称为构造块。构造块在每一次实例化类对象时都会被调用，而且由于构造方法执行。
* **静态块**：如果**一个代码块使用static进行定义**，就称其为静态块。在**类中的静态块会优先调用**，并且**只调用一次**。

## 3.16 内部类
* **内部类使用外部类属性**：`外部类.this.属性`
* **内部类对象实例化**：`外部类.内部类 对象 = new 外部类().new 内部类();`
* **使用static定义内部类**，则这个内部类不受外部类的影响，**实例化**方式为：`外部类.内部类 对象 = new 外部类.内部类();`

## 3.17 链表
**链表的基本操作**有如下特点：
1. **客户端**代码**不用关注具体的Node以及引用关系的细节**，**只需要关注Link类中提供的数据操作方法**。
2. **Link类**的主要功能是**控制Node类对象的产生和根节点**。
3. **Node类**主要负责**数据的保存以及引用关系的分配**。

开发可用链表的基础功能：
1. 向链表中增加新的数据：`public void add(数据类型 变量)`
2. 取得链表中保存的元素个数：`public int size()`
3. 判断是否是空链表：`public boolean isEmpty()`
4. 判断摸一个数据是否存在：`public boolean contains(数据类型 变量)`
5. 根据索引取得数据：`public 数据类型 get(int index)`
6. 使用新的内容替换指定索引的旧内容：`public void set(int index,数据类型 变量)`
7. 删除指定数据：`public void remove(数据类型 变量)`
8. 讲链表以对象数组的形式返回：`public 数据类型 [] toArray()`
...
class Link{
	...
	public void set(int index,String data){
		if (index > this.count - 1 || data == null){
			return;
		}
		this.foot = 0;
...
public class TestDemo{
	public static void main(String args[]){
		...
		link.add("丰收");
		link.add(null);
		System.out.println("链表中保存元素个数：" + link.size());
		System.out.println("是否存在“刘收”：" + link.contains("刘收"));
		System.out.println("索引为2的数据为：" + link.get(2));
		link.set(2,"收丰");
		link.remove("刘丰");
